...
class Employee:
    def __init__(self, root):
        ...
        # logo
        img_logo = Image.open("./CodingBlocks/images/icon.jpg")
        ...
        # 1st image
        img1 = Image.open("./CodingBlocks/images/img1.png")
        ...
        # 2nd image
        img2 = Image.open("./CodingBlocks/images/img3.png")
        ...
        # 3rd image
        img3 = Image.open("./CodingBlocks/images/img2.png")
        ...
        lbl_name = Label(
            upper_frame, text="Name", font=("arial", 11, "bold"), bg="white"
        )
        lbl_name.grid(row=0, column=2, sticky=W)

        entry_name = Entry(
            upper_frame, textvariable=self.name, font=("arial", 11, "bold"), width=20
        )
        entry_name.grid(row=0, column=3, padx=2, sticky=W)

        # label for entry field for phone
        lbl_phone = Label(
            upper_frame, text="Phone", font=("arial", 11, "bold"), bg="white"
        )
        lbl_phone.grid(row=0, column=4, sticky=W)

        entry_phone = Entry(
            upper_frame, textvariable=self.phone, font=("arial", 11, "bold"), width=20
        )
        entry_phone.grid(row=0, column=5, padx=2, sticky=W)

        # label for entry field for email
        lbl_email = Label(
            upper_frame, text="Email", font=("arial", 11, "bold"), bg="white"
        )
        lbl_email.grid(row=1, column=0, sticky=W)

        entry_email = Entry(
            upper_frame, textvariable=self.email, font=("arial", 11, "bold"), width=20
        )
        entry_email.grid(row=1, column=1, padx=2, sticky=W)

        # label for entry field for address
        lbl_address = Label(
            upper_frame, text="Address", font=("arial", 11, "bold"), bg="white"
        )
        lbl_address.grid(row=1, column=2, sticky=W)

        entry_address = Entry(
            upper_frame, textvariable=self.address, font=("arial", 11, "bold"), width=20
        )
        entry_address.grid(row=1, column=3, padx=2, sticky=W)

        # label for entry field for country
        lbl_country = Label(
            upper_frame, text="Country", font=("arial", 11, "bold"), bg="white"
        )
        lbl_country.grid(row=1, column=4, sticky=W)

        entry_country = Entry(
            upper_frame, textvariable=self.country, font=("arial", 11, "bold"), width=20
        )
        ...
        entry_DOB = Entry(
            upper_frame, textvariable=self.dob, font=("arial", 11, "bold"), width=20
        )
        ...
        entry_DOJ = Entry(
            upper_frame, textvariable=self.doj, font=("arial", 11, "bold"), width=20
        )
        ...
        entry_CTC = Entry(
            upper_frame, textvariable=self.salary, font=("arial", 11, "bold"), width=20
        )
        ...
        lbl_gender = Label(
            upper_frame, text="Gender", font=("arial", 11, "bold"), bg="white"
        )
        ...
        lbl_id_type = ttk.Combobox(
            upper_frame,
            textvariable=self.idType,
            font=("arial", 11, "bold"),
            width=20,
            state="readonly",
        )
        ...
        entry_id_proof = Entry(
            upper_frame, textvariable=self.idProof, font=("arial", 11, "bold"), width=20
        )
        entry_id_proof.grid(row=4, column=1, padx=2, pady=7, sticky=W)

        # Add Employee Image
        employeeImg = Image.open("./CodingBlocks/images/employeeImage.png")
        ...
        btn_reset = Button(
            button_frame,
            command=self.resetEmployeeInfo,
            text="Reset",
            font=("arial", 12, "bold"),
            fg="white",
            bg="blue",
        ...
        btn_search = Button(
            search_frame,
            command=self.searchEmployeeInfo,
            text="Search",
            font=("arial", 12, "bold"),
            fg="white",
            bg="blue",
    ...
    def AddData(self):
        ...
        try:
            cursor.execute(
                "insert into samra values (%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)",
                (
                    self.department.get(),
                    self.name.get(),
                    self.phone.get(),
    ...
    def ShowAll(self):
        ...
        cursor = conn.cursor()
        try:
            cursor.execute("select * from samra")
    ...
    def deleteData(self):
        conn = mysql.connector.connect(
            host="localhost", username="root", password="root", database="MCA"
        )
        cursor = conn.cursor()

        try:
            cursor.execute(
                "delete from samra where IdProof= (%s)", (self.idProof.get(),)
            )
            conn.commit()
            messagebox.showinfo(
    ...
    def searchEmployeeInfo(self):
        ...
        try:
            cursor.execute(
                "select * from samra where " + str(self.searchBy.get()) + " LIKE %s",
                (self.searchText.get(),),
            )
            data = cursor.fetchall()
    ...
    def updateData(self):
        ...
        try:
            cursor.execute(
                "update samra set Department=%s,Name=%s,Phone=%s,Email=%s,DOB=%s,DOJ=%s,Salary=%s,IdType=%s,Married=%s,Gender=%s,Address=%s,Country=%s where IdProof=%s",
                (
                    self.department.get(),
                    self.name.get(),
                    self.phone.get(),
